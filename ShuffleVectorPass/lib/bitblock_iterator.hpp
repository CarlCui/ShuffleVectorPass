...
template <class bitblock_t, class scanblock_t>
class BitBlockStreamScanner {

public:

BitBlockStreamScanner():  bitblock_ary(NULL),
                          bitblock_cnt(-1), 
                          bitblock_idx(-1), 
                          scanblock_idx(-1)
{
  current._bitblock = simd<1>::constant<0>();
}

void init(bitblock_t *s, uint32_t size) {
  bitblock_ary           = s;
  bitblock_cnt           = size;
  bitblock_idx           = 0;
  scanblock_idx          = 0;
  current._bitblock      = bitblock_ary[0];
}
...
int scan_to_next() {
  ...
  while(1) {
    ...
    for(scanblock_idx; scanblock_idx < (sizeof(bitblock_t)/sizeof(scanblock_t)); scanblock_idx++) {
      scanblock_t scan_item = current._scanblock[scanblock_idx];
      if(scan_item != 0) {
        int bit_pos = scan_forward_zeroes(scan_item);
        scan_item = scan_item & (scan_item - 1);
        current._scanblock[scanblock_idx] = scan_item;
        return (bitblock_idx * (sizeof(bitblock_t)*8)) + (scanblock_idx * sizeof(scanblock_t)*8) + bit_pos;
      }
    }
    ...
    if(bitblock_idx >= bitblock_cnt) {
      return -1;
    }
    current._bitblock = bitblock_ary[bitblock_idx];
  }
...
private:
  ...
  union {  bitblock_t   _bitblock;
           scanblock_t  _scanblock[sizeof(bitblock_t)/sizeof(scanblock_t)];} current;

  bitblock_t * bitblock_ary;
  uint32_t bitblock_cnt;
  uint32_t bitblock_idx;
  uint32_t scanblock_idx;  
